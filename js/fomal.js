...
function tonav() {
  ...
  document.getElementById("page-name").innerText = document.title.split(" | MoyunLee")[0];
}
...
$.ajax({
  ...
  data: {
    key: '25RBZ-VDR6J-DDIFJ-KZ7Y3-2XFIV-LABZT',  // 这里要写你的KEY!!!
    output: 'jsonp',
  },
  dataType: 'jsonp',
...
function createtime1() {
  var grt = new Date("09/01/2024 00:00:00"); //此处修改你的建站时间或者网站上线时间
...
function createtime2() {
  ...
  setTimeout(
    console.warn.bind(
      console,
      "%c ⚡ Powered by MoyunLee %c 你正在访问MoyunLeeの小家",
...
function share_() {
  ...
  try {
    ...
    var subTitle = title.endsWith("| MoyunLee") ? title.substring(0, title.length - 14) : title;
    navigator.clipboard.writeText('MoyunLee的站内分享\n标题：' + subTitle + '\n链接：' + url + '\n欢迎来访！🍭🍭🍭');
    new Vue({
      data: function () {
        this.$notify({
          title: "成功复制分享信息🎉",
...
if (localStorage.getItem("blogbg") != undefined) {
  ...
  document.getElementById("defineBg").innerText = `:root{
    --default-bg: url(https://tuchuang.voooe.cn/images/2024/10/28/bj.jpg);
    --darkmode-bg:url(https://tuchuang.voooe.cn/images/2024/10/28/bj.jpg);
    --mobileday-bg: url(https://tuchuang.voooe.cn/images/2024/10/28/bj.jpg);
    --mobilenight-bg: url(https://tuchuang.voooe.cn/images/2024/10/28/bj.jpg);
  }`;
}
// 切换背景主函数
function changeBg(s) {
